...
const showList = async () => {
    ...
    const res = await fetch("/adminMostFav/getMostFav", {
        method: "GET",
        headers: { 'Content-Type': 'application/json' }
    });
    const data = await res.json();
    console.log(data.mostFavs);
    if (data.mostFavs) {
        const array = data.mostFavs;
...
const search = async () => {
    ...
    searchBtn.addEventListener("click", async (e) => {
        ...
        if (toSearch.length != 0) {
            const res = await fetch("/adminMostFav/search", {
                method: "POST",
