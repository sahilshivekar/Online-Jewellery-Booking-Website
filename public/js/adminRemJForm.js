...
const submitFormValues = async () => {
    ...
    updateJewelleryForm.addEventListener('submit', async (event) => {
        ...
        const confirm = window.confirm("are you sure you want to remove this jewellery?");
        console.log(confirm);
        if (confirm == true) {
            ...
            const res = await fetch('adminRemJForm/deleteJ', {
                method: 'DELETE',
                headers: {
            ...
            if (res.ok) {
                ...
                alert("Jewellery is removed successfully");
                location.assign("/adminRemJ");
            } else {
                alert('Failed to remove jewellery. Try again!');
            }
...
const setSize = async () => {
    ...
    if (catVal == 'bracelets') {
        ...
        <input type="number" value="${sizeVal}" id="size" name="size" readonly>
        `
...
const disable = async () => {
    ...
    // Loop through each checkbox and disable user interaction
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('click', (e) => {
            e.preventDefault()
            return false; // Prevent default checkbox behavior
        });
    });
    ...
    selectElements.forEach(select => {
        // Set the disabled attribute for the select element
        select.disabled = true;

        // Disable all options within the select element
